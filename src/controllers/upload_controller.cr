class UploadController < ApplicationController
  def index
    if context.files.has_key?("file") && !context.files["file"].filename.nil?
      fname = context.files["file"].filename.to_s
      ...
      ::File.open("./public#{new_full_name}", "w") do |f|
        ::IO.copy(context.files["file"].file, f)
      end
      context.files["file"].file.unlink
